#!/usr/bin/env node
/**
 * Fix chapters with gmbr.pro images/thumbnails (403 Cloudflare block)
 * ──────────────────────────────────────────────────────────────────────
 * Re-downloads images from source and uploads to R2.
 *
 * Usage:
 *   node scripts/fix-gmbr-chapter.mjs --chapter-id=UUID    # fix one chapter
 *   node scripts/fix-gmbr-chapter.mjs --manga=SLUG          # fix all chapters of a manga
 *   node scripts/fix-gmbr-chapter.mjs --all                 # fix ALL gmbr.pro chapters
 *   node scripts/fix-gmbr-chapter.mjs --dry-run             # preview only
 */
...
const FAILURES_FILE = path.join(__dirname, 'failed-gmbr-fix.jsonl');
...
const CHAPTER_ID = args['chapter-id'] ?? null;
...
async function main() {
  console.log('');
  console.log('🔧  Fix gmbr.pro Chapters');
  console.log('════════════════════════════════════════');
  if (CHAPTER_ID) console.log(`   Chapter ID  : ${CHAPTER_ID}`);
  if (MANGA_SLUG) console.log(`   Manga slug  : ${MANGA_SLUG}`);
  if (FIX_ALL)    console.log(`   Mode        : ALL gmbr.pro chapters`);
  ...
  if (CHAPTER_ID) {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error || !data) {
      console.error('❌ Chapter tidak ditemukan:', error?.message ?? 'not found');
      process.exit(1);
    }
    chapters = [data];
  } else {
    // Find chapters with gmbr.pro images
    let query = supabase
      .from('chapters')
      .select('id, number, manga_id, thumbnail_url')
      .is('deleted_at', null)
    ...
    if (MANGA_SLUG) {
      ...
      const { data: manga } = await supabase
        ...
        .maybeSingle();
      if (!manga) {
        console.error('❌ Manga tidak ditemukan:', MANGA_SLUG);
        process.exit(1);
      }
      query = query.eq('manga_id', manga.id);
    }
  ...
  console.log(`📊 Found ${chapters.length} chapters with gmbr.pro thumbnails`);
  console.log('');

  if (chapters.length === 0) {
    console.log('✅ No gmbr.pro chapters to fix!');
    return;
  }
  ...
  for (const chapter of chapters) {
    ...
    if (!manga?.source_url) {
      console.log(`  Ch.${chapter.number}: ❌ No source_url`);
      failed++;
      continue;
    }
    ...
    if (DRY_RUN) {
      console.log(`    [DRY] Would delete gmbr.pro images + re-download from source`);
      continue;
    }

    // Step 1: Delete existing gmbr.pro images for this chapter
    ...
    if (delErr) {
      console.log(`    ⚠️  Delete gmbr.pro images failed: ${delErr.message}`);
    }

    // Step 2: Derive chapter URL from source_url
    ...
    // Step 3: Fetch chapter page and parse images
    ...
    // Step 4: Download & upload images in batches of 3
    const BATCH = 3;
    const imageRecords = [];
